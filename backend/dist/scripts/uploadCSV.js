...
async function main() {
    ...
    const csvPath = path_1.default.join(__dirname, "../dataset.csv");
    ...
    const copyStream = client.query((0, pg_copy_streams_1.from)(`
      COPY "transactions" (
    ...
    let isFirstRow = true;
    const transformStream = (0, fast_csv_1.parse)({ headers: true, ignoreEmpty: true, trim: true })
        .transform((row) => {
        if (isFirstRow) {
    ...
    const formatStream = (0, fast_csv_1.format)({ headers: columns, quote: '"' });
    fileStream
